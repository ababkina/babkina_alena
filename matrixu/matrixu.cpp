...
MatrixU::MatrixU() 
    :nRow(defaultRow)
    , nCol(defaultCol)
    , pData_(new int[defaultRow * defaultCol] { int() } )
{
}

MatrixU::MatrixU(const int row, const int col) 
    : nRow(row)
    , nCol(col)
{
    if ((row <= 0) || (col <= 0))
    {
        throw invalid_argument("Error: Size can't be less or equals 0");
    }
...
MatrixU::MatrixU(const MatrixU& rhs) 
    :nRow(rhs.nRow)
    , nCol(rhs.nCol)
    , pData_(new int[rhs.nRow * rhs.nCol] { int() } )
{
...
int& MatrixU::at(const ptrdiff_t iRow, const ptrdiff_t iCol)
{
    ...
    {
        throw invalid_argument("Error2: Out of range"); 
    }
...
const int& MatrixU::at(const ptrdiff_t iRow, const ptrdiff_t iCol) const
{
    if ((iRow < 0) || (iCol < 0) || (iRow >= nRow) || (iCol >= nCol)) 
    { 
        throw invalid_argument("Error3: Out of range"); 
    }
    return pData_[nCol * iRow + iCol];
}
